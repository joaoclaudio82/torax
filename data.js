...
export function studyFromNihEntry(entry) {
  ...
  return {
    ...
    image: entry.path,
    alt: `Radiografia NIH ${labels.join(", ") || entry.id}`,
    badge: "NIH demo",
    description:
      entry.description ||
      "Caso educacional do ChestX-ray14. Rótulos minerados por NLP, não constituem laudo.",
    observations: [
      `Rótulos NIH: ${labels.join("|") || "—"}`,
      `Incidência ${entry.view || "não informada"}`,
      "Rótulos text-mined — valide visualmente antes de interpretar o modelo",
    ],
    learningTags: ["NIH", ...labels],
    source: entry.source || "https://nihcc.app.box.com/v/ChestXray-NIHCC",
    license: "NIH Clinical Center (atribuição obrigatória)",
  };
}

export function mergeNihStudies(baseStudies, nihEntries) {
  const withoutNih = baseStudies.filter((study) => !study.id.startsWith("nih-"));
  return [...withoutNih, ...nihEntries.map(studyFromNihEntry)];
}
